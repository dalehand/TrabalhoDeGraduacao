...
int CONNECTION::link_consumer(UCHAR request[])
{
    ...
    if (instance == IO_POLL)
    {
        ...
        if (request[LENGTH] == 1) return 2;
        else if (request[LENGTH] == 0) return 1;
...
void ASSEMBLY::handle_io_poll_request(UCHAR request[], UCHAR response[])
{
    ...
    {
        memset(response, 0, BUFSIZE);
        memcpy(response, data, NUM_IN);
        response[LENGTH] = NUM_IN;
    }
}
...
public:
    ...
    ROUTER(DISCRETE_INPUT_POINT *in, DISCRETE_OUTPUT_POINT *out,IDENTITY *id,
           DEVICENET *dn, CONNECTION *ex, CONNECTION *io, ASSEMBLY *asin, ASSEMBLY *asout)
    {
    ...
    while(1)
    {
        ...
        if ((e & IO_POLL_REQUEST) && !(e & (IO_POLL_REQUEST - 1)))
        {
            ...
            if ((global_status & ON_LINE) && !(global_status & NETWORK_FAULT))
            {
                ...
                assembly_type = io_poll.link_consumer(request);
                if (assembly_type == 1)  	// try to consume the request
                {
                    assembly_in.handle_io_poll_request(request, response);
                    io_poll.link_producer(response); 	// produce response
                }else if (assembly_type == 2)
                {
