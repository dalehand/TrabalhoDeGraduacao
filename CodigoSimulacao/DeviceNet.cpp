...
void DISCRETE_INPUT_POINT::handle_explicit(UCHAR request[], UCHAR response[])
{
    ...
    error = 0;
    memset(response, 0, BUFSIZE);
    printf("\nDISCRETE INPUT POINT HANDLE EXPLICIT\n");
    ...
    case GET_REQUEST:
        ...
        {
        case 3:  // value
            printf("\nDISCRETE INPUT POINT HANDLE EXPLICIT: GET VALUE\n");
            response[0] = request[0] & NON_FRAGMENTED;
            response[1] = service | SUCCESS_RESPONSE;
            response[2] = value;
            response[LENGTH] = 3;
        ...
        case 4:  // status
            printf("\nDISCRETE INPUT POINT HANDLE EXPLICIT: GET STATUS\n");
            response[0] = request[0] & NON_FRAGMENTED;
            response[1] = service | SUCCESS_RESPONSE;
...
void DISCRETE_OUTPUT_POINT::handle_class_inquiry(UCHAR request[], UCHAR response[])
{
    ...
    error = 0;
    memset(response, 0, BUFSIZE);
    printf("\nDISCRETE OUTPUT POINT HANDLE CLASS \n");
...
void DISCRETE_OUTPUT_POINT::handle_explicit(UCHAR request[], UCHAR response[])
{
    ...
    error = 0;
    memset(response, 0, BUFSIZE);
    printf("\nDISCRETE OUTPUT POINT HANDLE EXPLICIT \n");
    switch(service)
    {
    case GET_REQUEST:
        switch(attrib)
        {
        case 3:  // value
            printf("\nDISCRETE OUTPUT POINT HANDLE EXPLICIT: GET VALUE\n");
            response[0] = request[0] & NON_FRAGMENTED;
            response[1] = service | SUCCESS_RESPONSE;
        ...
        case 4:  // status
            printf("\nDISCRETE OUTPUT POINT HANDLE EXPLICIT: GET STATUS \n");
            response[0] = request[0] & NON_FRAGMENTED;
            response[1] = service | SUCCESS_RESPONSE;
            response[2] = (UCHAR)status;
            response[LENGTH] = 3;
    ...
    case SET_REQUEST:
        ...
        {
        case 3: // set value
            printf("\nDISCRETE OUTPUT POINT HANDLE EXPLICIT: SET VALUE\n");
            set_value(request[5]);
            response[0] = request[0] & NON_FRAGMENTED;
            response[1] = service | SUCCESS_RESPONSE;
            response[LENGTH] = 2;
...
void ROUTER::handle_explicit(UCHAR request[], UCHAR response[])
{
    ...
    printf("\n\n ROUTER DOESNT SUPPORT THIS SERVICE\n\n");
    ...
    case DISCRETE_INPUT_POINT_CLASS:
        if(instance == 0){
            ...
            while((i != -1)&&( i <= NUM_IN))
            {
                if(instance == i)
                {
                    printf("\n\n DISCRETE INPUT POINT INSTANCE %d\n\n",i);
    ...
    case DISCRETE_OUTPUT_POINT_CLASS:
        if(instance == 0){
            ...
            while((i != -1)&&( i <= NUM_OUT))
            {
                if(instance == i)
                {
                    printf("\n\n DISCRETE OUTPUT POINT INSTANCE %d\n\n",i);
...
// 0010 0000 0000 0100 / 0010 0100 0000 0001 / 0011 0000 0000 0011 (Apendix C - CIPv1)
// 001 - Logical Segment
// 000 - Class ID
...
// 0010 0000 0000 0100 / 0010 1000 0000 0001 / 0011 0000 0000 0011 (Apendix C - CIPv1)
// 001 - Logical Segment
// 000 - Class ID
// 00 - 8-bit logical address
// 00000100 - Class ID #4  => Assembly Object (CIPv1-5.43)
